  ...
  <div>
    ...
    <kindo-box title="项目超限定频次规则信息">
      <kindo-table ref="table" :url="table.url" :default-sort="tableSort" :queryParam="search"
        ...
        <el-table-column label="组号" prop="groupingCode" width="140" header-align="center" sortable='custom'
          show-overflow-tooltip></el-table-column>
        <el-table-column label="组名称" prop="groupingName" width="140" header-align="center" sortable='custom'
          show-overflow-tooltip></el-table-column>
        <el-table-column label="诊疗项目编码" prop="itemCode" width="140" header-align="center" sortable='custom'
          show-overflow-tooltip></el-table-column>
        <el-table-column label="诊疗项目名称" prop="itemName" width="140" header-align="center" sortable='custom'
          show-overflow-tooltip></el-table-column>
        <el-table-column label="单位" prop="unit" width="90" align="center" header-align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column label="住院每日次数" prop="hospitalizedNum" width="130" align="right" header-align="center"
          sortable='custom' show-overflow-tooltip></el-table-column>
        <el-table-column label="住院限定次数" prop="hospitalizedLimitNum" width="130" align="right" header-align="center"
          sortable='custom' show-overflow-tooltip></el-table-column>
        <el-table-column label="计算方式" prop="computeMode"
          :formatter="(r,c,v) => kindo.dictionary.getLabel(dict.FACILITY_TYPE ,v)" width="140" align="left"
          header-align="center" sortable='custom' show-overflow-tooltip></el-table-column>
    ...
    <el-dialog v-drag top="0" :visible.sync="visible" :title="(form.id?'编辑':'新增') +'诊疗项目'"
      ...
      <el-form :model="form" label-position="right" ref="form" class="box" label-width="110px" :rules="formRules">
        ...
        <el-form-item label="住院每日次数" prop="hospitalizedNum">
          <el-input-number :controls="false" :min="0" :max="99" v-model.trim="form.hospitalizedNum"></el-input-number>
        </el-form-item>
        <el-form-item label="住院限定次数" prop="hospitalizedLimitNum">
          <el-input-number :controls="false" :min="0" :max="99" v-model.trim="form.hospitalizedLimitNum">
          </el-input-number>
        </el-form-item>
        <el-form-item label="计算方式" prop="computeMode">
          <el-select v-model="form.computeMode" :disabled="form.id === '' && form.groupingCode !== ''" placeholder="请选择"
            clearable filterable>
            <el-option v-for="(item,index) in dict.FACILITY_TYPE" :key="index" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
  ...
  data() {
    return {
      ...
      form: {
        ...
        groupingCode: '',
        itemCode: '',
        hospitalizedNum: undefined,
        hospitalizedLimitNum: undefined,
        computeMode: '',
        remark: ''
      },
      formRules: {
  ...
  methods: {
    ...
    addRow(row) {
      kindo.util.promise(() => {
        ...
        this.form.computeMode = row.computeMode
      })
    },
    // 编辑
    editRow(row) {
    ...
    saveData() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          let itemArr = ['hospitalizedNum', 'hospitalizedLimitNum']
          itemArr.forEach(item => {
            if (params[item] === undefined) {
              params[item] = null
            }
          })
          this.$http[requestType](mainUrl, params).then(res => {
