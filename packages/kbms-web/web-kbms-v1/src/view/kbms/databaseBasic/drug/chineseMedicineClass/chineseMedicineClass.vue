...
<template>
  <div>
    <kindo-box title="中成药分类" icon="xx" style="margin-bottom:0px;">
      ...
      <el-tree :style="treeHeight" :default-expand-all="true" :data="treeData" node-key="id" id="drugCatelogTree" ref="tree" :highlight-current="true" :default-expanded-keys="openKeys" :props="treeProp" :filter-node-method="filterNode" :expand-on-click-node="false">
        <span class="custom-tree-node" slot-scope="{node, data}">
          ...
          <span style="color: #409EFF; padding-left:4px;">
            <span class="el-icon-plus" v-if="node.level === 1" @click="append(node, data)">
            </span>
            <span @click="modify(node, data)" v-if="node.level !== 1" class="el-icon-edit-outline">
            </span>
            <span @click="remove(node, data)" v-if="node.level !== 1 && node.isLeaf" class="el-icon-minus">
            </span>
          </span>
        </span>
      </el-tree>
      <div slot="control">
        <el-button icon="el-icon-plus" type="primary" v-if="treeData.length < 3" @click="append()">新增</el-button>
      </div>
    </kindo-box>
    <!-- 新增节点弹出框starts-->
    <el-dialog top="0" :visible.sync="visible2" :title="dialogTitle2" :modal-append-to-body="false" :close-on-click-modal="false">
      <el-form :model="dialog2" ref="dialog2Form" label-position="right" label-width="130px" onsubmit="return false;" :rules="dialog2Rules">
        <el-form-item label="中成药分类名称" style="display:block;" prop="drugCategoryName">
