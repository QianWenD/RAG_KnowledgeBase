  ...
  <div>
    ...
    <kindo-box title="给药途径信息">
      <kindo-table ref="table" :url="url" :queryParam="search" @selection-change="(selection) => selectionChange(selection, 'selection')" @filter-change="(filters) => filterChange(filters, 'table', 'search')">
        ...
        <el-table-column label="名称" fixed="left" prop="name" min-width="120" header-align="center" sortable='custom' show-overflow-tooltip></el-table-column>
        <el-table-column label="备注" prop="remark" min-width="160" header-align="center" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog v-drag top="0" :visible.sync="visible" :title="(form.id?'编辑':'新增') +'给药途径'" :modal-append-to-body="false" :close-on-click-modal="false">
      <el-form :model="form" :rules="formRules" onsubmit="return false;" label-width="110px" ref="form">
        <el-form-item label="给药途径名称" prop="name">
          <el-input v-model.trim="form.name" clearable></el-input>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input type="textarea" :rows="2" placeholder="可输入200字" v-model.trim="form.remark">
          </el-input>
        </el-form-item>
      </el-form>
