  ...
  <div>
    ...
    <kindo-box title="康复理疗规则">
      <kindo-table ref="table" :url="table.url" :queryParam="search" @selection-change="(selection) => selectionChange(selection, 'selection')" @filter-change="(filters)=>filterChange(filters,'table', 'search')" :default-sort="tableSort">
        ...
        <el-table-column label="诊疗项目编码" prop="itemCode" width="130" header-align="center" sortable='custom' show-overflow-tooltip></el-table-column>
        <el-table-column label="诊疗项目名称" prop="itemName" min-width="140" header-align="center" sortable='custom' show-overflow-tooltip></el-table-column>
    ...
    <el-dialog v-drag top="0" :visible.sync="visible" :title="(form.id?'编辑':'新增') +'康复理疗规则'" :modal-append-to-body="false" :close-on-click-modal="false">
      <el-form :model="form" label-position="right" ref="form" class="box" label-width="120px" :rules="formRules">
        ...
        <el-form-item label="政策限定用量(次)" prop="itemNum">
          <el-input-number v-model.trim="form.itemNum" size="mini" :controls="false" :min="0" :max="99" class="inputNumber"></el-input-number>
        </el-form-item>
      </el-form>
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        itemNum: [{ required: true, message: '不能为空', trigger: 'blur' }]
      },
      visible: false
    }
  },
