  ...
  <div>
    ...
    <kindo-box title="诊疗项目超计价单位收费规则">
      <kindo-table ref="table" :url="table.url" :queryParam="search" :default-sort="tableSort" @selection-change="(selection) => selectionChange(selection, 'selection')" @filter-change="(filters)=>filterChange(filters,'table', 'search')">
        ...
        <el-table-column label="住院每日次数" prop="itemNum" width="140" align="center" header-align="center" sortable='custom' show-overflow-tooltip></el-table-column>
    ...
    <el-dialog v-drag top="0" :visible.sync="visible" :title="(form.id?'编辑':'新增') +'诊疗项目'" :modal-append-to-body="false" :close-on-click-modal="false">
      <el-form :model="form" label-position="right" ref="form" class="box" label-width="120px" :rules="formRules">
        ...
        <el-form-item label="住院每日次数" prop="itemNum">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      formRules: {
        itemCode: [{ required: true, message: '请输入选择', trigger: 'blur' }],
        itemNum: [
