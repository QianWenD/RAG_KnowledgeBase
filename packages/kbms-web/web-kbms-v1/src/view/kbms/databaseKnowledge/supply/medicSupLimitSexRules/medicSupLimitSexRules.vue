  ...
  <div>
    <kindo-box title="查询条件" icon="xx">
      <el-form :model="search" label-position="right" onsubmit="return false;" inline @keyup.enter.prevent.native="get('table')">
        <el-form-item label="医疗耗材">
          <el-input v-model.trim="search.itemKey" clearable placeholder="请输入关键字"></el-input>
        </el-form-item>
    ...
    <kindo-box title="医用耗材限性别规则信息">
      <kindo-table ref="table" :url="table.url" :default-sort="tableSort" :queryParam="search" @selection-change="(selection) => selectionChange(selection, 'selection')" @filter-change="(filters)=>filterChange(filters,'table', 'search')">
        ...
        <el-table-column label="医疗耗材" prop="itemKey" width="140" header-align="center" sortable='custom' show-overflow-tooltip></el-table-column>
    ...
    <el-dialog v-drag top="0" :visible.sync="visible" :title="(form.id?'编辑':'新增') +'限性别耗材'" :modal-append-to-body="false" :close-on-click-modal="false">
      <el-form :model="form" label-position="right" ref="form" class="box" label-width="90px" :rules="formRules">
        <el-form-item label="医疗耗材" style="display:block;" prop="itemKey">
          <el-input v-model.trim="form.itemKey"></el-input>
        </el-form-item>
        ...
        <el-form-item label="描述" prop="remark">
          <el-input type="textarea" :rows="2" placeholder="可以输入200字" v-model.trim="form.remark"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        itemKey: [{ required: true, message: '请输入', trigger: 'blur' }, { min: 0, max: 30, message: '长度不能超过30' }],
