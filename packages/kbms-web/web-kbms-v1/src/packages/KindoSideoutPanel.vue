    ...
    <transition name="slide-out" v-on:enter="d_enter">
      <div v-if="isVisibleDock" ref="dock" class="dock" :style="_style(0)" :class="_class(0)">
        <transition-group name="slide-out" v-on:before-leave="s_beforeLeave" v-on:afterLeave="s_afterLeave">
          <template v-for="(isVisible, i) in isVisibleSections">
            <section v-if="isVisible" ref="section" class="panel" :key="'k'+i" :style="_style(i+1)" :class="_class(i+1)">
    ...
    init() {
      ...
      for (let i = 0; i < 5; i++) {
        if (this.classes[i] && this.classes[i].length >= 0) {
          i == 2 && this.classes[i] == 'same' ? this.d_classes.push(this.classes[i - 1]) : this.d_classes.push(this.classes[i])
        } else {
